...
public abstract class ArbolBinario<T> implements Coleccionable<T> {
    ...
    /**
     * Metodo auxiliar para equals que recorre los arboles en postorder
     * 
     * @param original
     * @param comparacion
     * @return si dos nodos son iguales
     */
    public boolean equalsAux(Nodo original, Nodo comparacion) {
        if (original.hayIzquierdo())
            equalsAux(original.izquierdo, comparacion.izquierdo);

        if (original.hayDerecho())
            equalsAux(original.derecho, comparacion.derecho);

        return original.equals(comparacion);
    }
