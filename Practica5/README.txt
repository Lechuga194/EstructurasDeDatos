Para ejecutar el codigo
1.- Abrir una terminal dentro de la carpeta src
2.- Ejecutar "javac *.java"
3.- Ejecutar  "java PruebaArbolOrdenado"

AVL

*actualizaAltura 
    aqui verificamos que el nodo no sea nulo, si no lo es guardamos la altura de su hijo izquierdo y derecho (0 en caso de que no existan)
    y asignamos la altura del nodo  al maximo de estos + 1

*Rebalancea toma dos posibles casos con un subcaso opcional para cada uno

*Agregar crea el nodo, agrega el nodo y Rebalancea

*Eliminar crea un nodo con el nodo requerido, lo elimina y Rebalancea

